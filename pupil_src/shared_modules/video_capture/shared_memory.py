...
class Shared_Memory(Base_Source):
    ...
    def get_frame(self):
        if self.shm:
          ...
          if(self.frame_size[0] * self.frame_size[1] * 4 != len(buf)):
            logger.error("Incorrect image dimension:{},{}. Size of buf {}".format(self.frame_size[0],self.frame_size[1],len(buf)))
            self.healthy = False
          else:
    ...
    def ui_elements(self):
        ...
        ui_elements.append(ui.Text_Input("frame_size", self, label="(width,height)"))
